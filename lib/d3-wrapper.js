...
const createButton = () => {
  // Create the ktns/log tabbed grouping, put it in the observe button DOM node.
  ...
  // Re-arrange the groupings of DOM elements in the right column so that it's easier for ktns to
  // manage their visibility.
  const logDiv = document.createElement('div');
  logDiv.setAttribute('id', 'log-group');
  logDiv.appendChild(document.querySelector('div.right-tab-header'));
  logDiv.appendChild(document.getElementById('rightTabLog'));
  logDiv.appendChild(document.getElementById('rightTabChat'));
  document.getElementById('rightColumn').appendChild(logDiv);
  return div;
};
...
function initD3(resources) {
  ...
  // A helper function for setting the chart visible/hidden via CSS classes
  const setChartActive = active => {
    ...
    logGroup.className = active ? 'disabled' : '';
  }
